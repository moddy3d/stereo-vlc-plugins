    ...
    if (supports_shaders) {
        char *code = NULL;
            ...
            const char *template_glsl_yuv =
                ...
                " float omega = height - gl_FragCoord.y - 1; "
                " float v_view = -1 * floor(mod(floor(omega * 3.0 * 0.222202850), 8.0));"
                " if (v_view == 0) "
                "   v_view = -8; "
                " float h_view = mod(gl_FragCoord.x * 3 + 8 - 2 + 1 - v_view, 8.0);"
                " float f_view[3] = float[3](h_view, h_view + 1.0, h_view + 2.0);"
                " int i_view[3]   = int[3](int(f_view[0]), int(f_view[1]), int(f_view[2]));"
                " for (int i = 0; i < 3; ++i) "
                " { "
                "   if (i_view[i] == 0) "
                "   { "
                ...
                " float oneeighths  = 1.0/8.0;"
                " float threeeighths = 3.0/8.0;"
                " float fiveeighths  = 5.0/8.0;"
                " float sixeighths   = 6.0/8.0;"
                " float v7_top_ratio = 1.0/3.0; "
                " float v7_bottom_ratio = v7_top_ratio;"
                " float v8_top_ratio = 2.0/3.0;"
                " float v8_bottom_ratio = 2.0/3.0;"
                " float _fix7 = 3.0/height; "
                // Init Samplers
                " vec4 r_smp[3] = vec4[3](   "
                ...
                "                 vec4(0.0, 0.0, 0.0, 0.0), "
                "                 vec4(0.0, 0.0, 0.0, 0.0)); "
                // Samplers for Red pixel
                " if (i_view[0] >= 1 && i_view[0] < 4) "
                " { "
                "     r_smp[0].x = TexCoord0.x / 3.0 + (onethird * float(i_view[0] - 1));"
                "     r_smp[0].y = TexCoord0.y * threeeighths;"
                "     r_smp[1].x = TexCoord1.x / 3.0 + (onethird * float(i_view[0] - 1));"
                "     r_smp[1].y = TexCoord1.y * threeeighths;"
                "     r_smp[2].x = TexCoord2.x / 3.0 + (onethird * float(i_view[0] - 1));"
                "     r_smp[2].y = TexCoord2.y * threeeighths;"
                " } "
                " else if (i_view[0] >= 4 && i_view[0] < 7) " 
                " { "
                "     r_smp[0].x = TexCoord0.x / 3.0 + (onethird * float(i_view[0] - 4));"
                "     r_smp[0].y = TexCoord0.y * threeeighths + (threeeighths);"
                "     r_smp[1].x = TexCoord1.x / 3.0 + (onethird * float(i_view[0] - 4));"
                "     r_smp[1].y = TexCoord1.y * threeeighths + (threeeighths);"
                "     r_smp[2].x = TexCoord2.x / 3.0 + (onethird * float(i_view[0] - 4));"
                "     r_smp[2].y = TexCoord2.y * threeeighths + (threeeighths);"
                " } "
                " else if (i_view[0] == 7) " 
                " { "
                "   if (TexCoord0.y >= 0.0 && TexCoord0.y < (v7_top_ratio))"
                "   { "
                "       float fix7 = 0; "
                "       float diff = v7_top_ratio - TexCoord0.y; "
                "       if (diff < _fix7) "
                "           fix7 = -(_fix7 - diff); "
                "       r_smp[0].x = TexCoord0.x / 3.0 + onethird;"
                "       r_smp[0].y = TexCoord0.y * threeeighths + (sixeighths) + fix7;"
                "       r_smp[1].x = TexCoord1.x / 3.0 + onethird;"
                "       r_smp[1].y = TexCoord1.y * threeeighths + (sixeighths) + fix7;"
                "       r_smp[2].x = TexCoord2.x / 3.0 + onethird;"
                "       r_smp[2].y = TexCoord2.y * threeeighths + (sixeighths) + fix7;"
                "     } "
                "     else if (TexCoord0.y >= (v7_bottom_ratio) && TexCoord0.y <= 1.0)"
                "     { "
                "       float fix7 = 0; "
                "       float diff = TexCoord0.y - v7_bottom_ratio; "
                "       if (diff < _fix7) "
                "           fix7 = _fix7 - diff; "
                "       r_smp[0].x = TexCoord0.x / 3.0;"
                "       r_smp[0].y = TexCoord0.y * threeeighths + fiveeighths + fix7;"
                "       r_smp[1].x = TexCoord1.x / 3.0;"
                "       r_smp[1].y = TexCoord1.y * threeeighths + fiveeighths + fix7;"
                "       r_smp[2].x = TexCoord2.x / 3.0;"
                "       r_smp[2].y = TexCoord2.y * threeeighths + fiveeighths + fix7;"
                "     } "
                " } "
                " else if (i_view[0] == 8) " 
                " { "
                "     if (TexCoord0.y >= 0.0 && TexCoord0.y < (v8_top_ratio))"
                "     { "
                "         r_smp[0].x = TexCoord0.x / 3.0 + twothirds;"
                "         r_smp[0].y = TexCoord0.y * threeeighths + sixeighths;"
                "         r_smp[1].x = TexCoord1.x / 3.0 + twothirds;"
                "         r_smp[1].y = TexCoord1.y * threeeighths + sixeighths;"
                "         r_smp[2].x = TexCoord2.x / 3.0 + twothirds;"
                "         r_smp[2].y = TexCoord2.y * threeeighths + sixeighths;"
                "     } "
                "     else if (TexCoord0.y >= v8_bottom_ratio && TexCoord0.y <= 1.0)"
                "     { "
                "         r_smp[0].x = TexCoord0.x / 3.0 + onethird;"
                "         r_smp[0].y = TexCoord0.y * threeeighths + (fiveeighths);"
                "         r_smp[1].x = TexCoord1.x / 3.0 + onethird;"
                "         r_smp[1].y = TexCoord1.y * threeeighths + (fiveeighths);"
                "         r_smp[2].x = TexCoord2.x / 3.0 + onethird;"
                "         r_smp[2].y = TexCoord2.y * threeeighths + (fiveeighths);"
                "     } "
                " } "
                " if (i_view[1] >= 1 && i_view[1] < 4) "
                " { "
                "     g_smp[0].x = TexCoord0.x / 3.0 + (onethird * float(i_view[1] - 1));"
                "     g_smp[0].y = TexCoord0.y * threeeighths;"
                "     g_smp[1].x = TexCoord1.x / 3.0 + (onethird * float(i_view[1] - 1));"
                "     g_smp[1].y = TexCoord1.y * threeeighths;"
                "     g_smp[2].x = TexCoord2.x / 3.0 + (onethird * float(i_view[1] - 1));"
                "     g_smp[2].y = TexCoord2.y * threeeighths;"
                " } "
                " else if (i_view[1] >= 4 && i_view[1] < 7) " 
                " { "
                "     g_smp[0].x = TexCoord0.x / 3.0 + (onethird * float(i_view[1] - 4));"
                "     g_smp[0].y = TexCoord0.y * threeeighths + (threeeighths);"
                "     g_smp[1].x = TexCoord1.x / 3.0 + (onethird * float(i_view[1] - 4));"
                "     g_smp[1].y = TexCoord1.y * threeeighths + (threeeighths);"
                "     g_smp[2].x = TexCoord2.x / 3.0 + (onethird * float(i_view[1] - 4));"
                "     g_smp[2].y = TexCoord2.y * threeeighths + (threeeighths);"
                " } "
                " else if (i_view[1] == 7) " 
                " { "
                "   if (TexCoord0.y >= 0.0 && TexCoord0.y < v7_top_ratio)"
                ...
                "       if (diff < _fix7) "
                "           fix7 = -(_fix7 - diff); "
                "       g_smp[0].x = TexCoord0.x / 3.0 + onethird;"
                "       g_smp[0].y = TexCoord0.y * threeeighths + (sixeighths) + fix7;"
                "       g_smp[1].x = TexCoord1.x / 3.0 + onethird;"
                "       g_smp[1].y = TexCoord1.y * threeeighths + (sixeighths) + fix7;"
                "       g_smp[2].x = TexCoord2.x / 3.0 + onethird;"
                "       g_smp[2].y = TexCoord2.y * threeeighths + (sixeighths) + fix7;"
                "     } "
                "     else if (TexCoord0.y >= v7_bottom_ratio && TexCoord0.y <= 1.0)"
                "     { "
                "       float fix7 = 0; "
                ...
                "       if (diff < _fix7) "
                "           fix7 = _fix7 - diff; "
                "         g_smp[0].x = TexCoord0.x / 3.0;"
                "         g_smp[0].y = TexCoord0.y * threeeighths + (fiveeighths) + fix7;"
                "         g_smp[1].x = TexCoord1.x / 3.0;"
                "         g_smp[1].y = TexCoord1.y * threeeighths + (fiveeighths) + fix7;"
                "         g_smp[2].x = TexCoord2.x / 3.0;"
                "         g_smp[2].y = TexCoord2.y * threeeighths + (fiveeighths) + fix7;"
                "     } "
                " } "
                " else if (i_view[1] == 8) " 
                " { "
                "     if (TexCoord0.y >= 0.0 && TexCoord0.y < v8_top_ratio)"
                "     { "
                "         g_smp[0].x = TexCoord0.x / 3.0 + ( twothirds);"
                "         g_smp[0].y = TexCoord0.y * threeeighths + (sixeighths);"
                "         g_smp[1].x = TexCoord1.x / 3.0 + ( twothirds);"
                "         g_smp[1].y = TexCoord1.y * threeeighths + (sixeighths);"
                "         g_smp[2].x = TexCoord2.x / 3.0 + ( twothirds);"
                "         g_smp[2].y = TexCoord2.y * threeeighths + (sixeighths);"
                "     } "
                "     else if (TexCoord0.y >= v8_bottom_ratio && TexCoord0.y <= 1.0)"
                "     { "
                "         g_smp[0].x = TexCoord0.x / 3.0 + onethird;"
                "         g_smp[0].y = TexCoord0.y * threeeighths + (fiveeighths);"
                "         g_smp[1].x = TexCoord1.x / 3.0 + onethird;"
                "         g_smp[1].y = TexCoord1.y * threeeighths + (fiveeighths);"
                "         g_smp[2].x = TexCoord2.x / 3.0 + onethird;"
                "         g_smp[2].y = TexCoord2.y * threeeighths + (fiveeighths);"
                "     } "
                " } "
                " if (i_view[2] >= 1 && i_view[2] < 4) "
                " { "
                "     b_smp[0].x = TexCoord0.x / 3.0 + (onethird * float(i_view[2] - 1));"
                "     b_smp[0].y = TexCoord0.y * threeeighths;"
                "     b_smp[1].x = TexCoord1.x / 3.0 + (onethird * float(i_view[2] - 1));"
                "     b_smp[1].y = TexCoord1.y * threeeighths;"
                "     b_smp[2].x = TexCoord2.x / 3.0 + (onethird * float(i_view[2] - 1));"
                "     b_smp[2].y = TexCoord2.y * threeeighths;"
                " } "
                " else if (i_view[2] >= 4 && i_view[2] < 7) " 
                " { "
                "     b_smp[0].x = TexCoord0.x / 3.0 + (onethird * float(i_view[2] - 4));"
                "     b_smp[0].y = TexCoord0.y * threeeighths + (threeeighths);"
                "     b_smp[1].x = TexCoord1.x / 3.0 + (onethird * float(i_view[2] - 4));"
                "     b_smp[1].y = TexCoord1.y * threeeighths + (threeeighths);"
                "     b_smp[2].x = TexCoord2.x / 3.0 + (onethird * float(i_view[2] - 4));"
                "     b_smp[2].y = TexCoord2.y * threeeighths + (threeeighths);"
                " } "
                " else if (i_view[2] == 7) " 
                " { "
                "   if (TexCoord0.y >= 0.0 && TexCoord0.y < v7_top_ratio)"
                ...
                "       if (diff < _fix7) "
                "           fix7 = -(_fix7 - diff); "
                "       b_smp[0].x = TexCoord0.x / 3.0 + onethird;"
                "       b_smp[0].y = TexCoord0.y * threeeighths + (sixeighths) + fix7;"
                "       b_smp[1].x = TexCoord1.x / 3.0 + onethird;"
                "       b_smp[1].y = TexCoord1.y * threeeighths + (sixeighths) + fix7;"
                "       b_smp[2].x = TexCoord2.x / 3.0 + onethird;"
                "       b_smp[2].y = TexCoord2.y * threeeighths + (sixeighths) + fix7;"
                "     } "
                "     else if (TexCoord0.y >= v7_bottom_ratio && TexCoord0.y <= 1.0)"
                "     { "
                "       float fix7 = 0; "
                ...
                "       if (diff < _fix7) "
                "           fix7 = _fix7 - diff; "
                "         b_smp[0].x = TexCoord0.x / 3.0;"
                "         b_smp[0].y = TexCoord0.y * threeeighths + (fiveeighths) + fix7;"
                "         b_smp[1].x = TexCoord1.x / 3.0;"
                "         b_smp[1].y = TexCoord1.y * threeeighths + (fiveeighths) + fix7;"
                "         b_smp[2].x = TexCoord2.x / 3.0;"
                "         b_smp[2].y = TexCoord2.y * threeeighths + (fiveeighths) + fix7;"
                "     } "
                " } "
                " else if (i_view[2] == 8) " 
                " { "
                "     if (TexCoord0.y >= 0.0 && TexCoord0.y < v8_top_ratio)"
                "     { "
                "         b_smp[0].x = TexCoord0.x / 3.0 + ( twothirds);"
                "         b_smp[0].y = TexCoord0.y * threeeighths + (sixeighths);"
                "         b_smp[1].x = TexCoord1.x / 3.0 + ( twothirds);"
                "         b_smp[1].y = TexCoord1.y * threeeighths + (sixeighths);"
                "         b_smp[2].x = TexCoord2.x / 3.0 + ( twothirds);"
                "         b_smp[2].y = TexCoord2.y * threeeighths + (sixeighths);"
                "     } "
                "     else if (TexCoord0.y >= v8_bottom_ratio && TexCoord0.y <= 1.0)"
                "     { "
                "         b_smp[0].x = TexCoord0.x / 3.0 + onethird;"
                "         b_smp[0].y = TexCoord0.y * threeeighths + (fiveeighths);"
                "         b_smp[1].x = TexCoord1.x / 3.0 + onethird;"
                "         b_smp[1].y = TexCoord1.y * threeeighths + (fiveeighths);"
                "         b_smp[2].x = TexCoord2.x / 3.0 + onethird;"
                "         b_smp[2].y = TexCoord2.y * threeeighths + (fiveeighths);"
                "     } "
                " } "
                " vec4 r_clr[4] = vec4[4](                   "
                "                 vec4(0.0, 0.0, 0.0, 0.0),  "
                ...
                "                 vec4(0.0, 0.0, 0.0, 0.0),  "
                "                 vec4(0.0, 0.0, 0.0, 0.0)); "
                " for (int yuv_cmp = 0; yuv_cmp < 3; ++yuv_cmp)"
                " { "
                "     int correct_yuv = yuv_cmp;"
                "     if (correct_yuv == 1) "
                "         correct_yuv = %c; "
                "     if (correct_yuv == 2) "
                "         correct_yuv = %c;"
                "     r_clr[correct_yuv] = texture2D(Texture[yuv_cmp], r_smp[yuv_cmp].xy); "
                "     g_clr[correct_yuv] = texture2D(Texture[yuv_cmp], g_smp[yuv_cmp].xy); "
                "     b_clr[correct_yuv] = texture2D(Texture[yuv_cmp], b_smp[yuv_cmp].xy); "
                " } "
                " r_clr[3] =  r_clr[0] * coefficient[0]  + coefficient[3];"
                " r_clr[3] = (r_clr[1] * coefficient[1]) + r_clr[3];"
                " r_clr[3] = (r_clr[2] * coefficient[2]) + r_clr[3];"
                " g_clr[3] =  g_clr[0] * coefficient[0]  + coefficient[3];"
                " g_clr[3] = (g_clr[1] * coefficient[1]) + g_clr[3];"
                " g_clr[3] = (g_clr[2] * coefficient[2]) + g_clr[3];"
                " b_clr[3] =  b_clr[0] * coefficient[0]  + coefficient[3];"
                " b_clr[3] = (b_clr[1] * coefficient[1]) + b_clr[3];"
                " b_clr[3] = (b_clr[2] * coefficient[2]) + b_clr[3];"
                " gl_FragColor.r = r_clr[3].r;"
                " gl_FragColor.g = g_clr[3].g;"
                " gl_FragColor.b = b_clr[3].b;"
                "}";
